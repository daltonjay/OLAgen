...
The OLAgen script is intended to take input sequences, determine differntiating
SNPs, and generate Oligonucleotide Ligation Assay probes for detection of the
determined SNPs.

The script follows methodologies described in Nelson, et al. "Ligation-based
Assay for Variant Typing without Sequencing: Application to SARS-CoV-2 Variants
of Concern". Influenza and Other Respiratory Viruses, (2022).

It heavily depends on the Python package "Biopython" (biopython.org).
"""
# Import the relevant tools for Biopython
import os
import Bio
...
# Make sure that I am in the right file. Edit this out after getting the tool working.
os.chdir('/Users/daltonjaynelson/Documents/Research/OLAgen')

''' ** COME BACK TO HERE FOR INPUT DATA, THIS STALLED OUT AND WASN'T WORKING JUST YET
SO EMPLOYING REPRESENTATIVE DATA FROM YOUTUBE FOR THE TIME BEING **
# First, the reference sequence(s) must be provided by the user as an input.
# ref_file = input('Provide the .fasta filename for the reference sequence and sequence of interest: ')
...
# # records = (rec.upper() for rec in SeqIO.parse(ref_file, "fasta"))
# # count = SeqIO.write(records, "references.fas", "fasta")
# # print("Converted %i records to upper case" % count)

# # # Align the reference sequence(s) and the sequence of interest

# # alignment = AlignIO.read(records,'stockholm')
# # print(alignment)  
'''
